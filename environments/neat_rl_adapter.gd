...
## Last reward read from the primary agent. Used to compute the per-step delta
## for envs where get_reward() returns a running total (e.g. bipedal_walker's
## _cached_reward is overwritten each step, so we accumulate the delta).
var _last_primary_reward: float = 0.0
...
func step_env() -> void:
	...
	# Accumulate the primary agent's reward delta.
	# For agents whose get_reward() returns a live-computed per-step value
	...
	if _primary_agent != null:
		var cur_reward: float = _primary_agent.get_reward()
		_cumulative_fitness += cur_reward - _last_primary_reward
		_last_primary_reward = cur_reward
