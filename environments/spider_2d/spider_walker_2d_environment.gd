...
func _compute_knee_world(leg_idx: int) -> Vector2:
        ...
        # Upper leg points DOWNWARD at angle=0, rotated by upper_angle.
        # +y is UP (matches visualizer convention), so downward = -cos(angle).
...
func step(action: Dictionary) -> Dictionary:
        ...
        # +y is UP, so feet are touching when at or below ground level + threshold.
