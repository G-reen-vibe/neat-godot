## Evaluates a population of genomes against an environment. Supports both
## single-threaded evaluation (for debugging) and multi-threaded evaluation
## via Godot's WorkerThreadPool.
##
## Each genome is evaluated by:
...
## Evaluate all genomes in [param genomes] and return an Array[float] of fitnesses
## (parallel to [param genomes]).
...
func _eval_task(genome: Genome, out: Array[float], index: int) -> void:
	out[index] = _evaluate_one(genome)

# Core evaluation logic for one genome.
func _evaluate_one(genome: Genome) -> float:
