...
##     PinJoint2D (anchor: fixed to world at origin) - between StaticBody2D and Link1
##     StaticBody2D (anchor point at origin)
##     RigidBody2D (Link1) - first link, hangs from anchor
##       CollisionShape2D (capsule, length 1.0)
##     RigidBody2D (Link2) - second link, hangs from Link1's tip
##       CollisionShape2D (capsule, length 1.0)
##     PinJoint2D (joint1: between Link1 and Link2, with motor for torque)
##
...
@onready var _link1: RigidBody2D = $Link1
@onready var _link2: RigidBody2D = $Link2
