...
##     currently disabled. Safe by design: disabled connections don't
##     contribute to the functional graph, so removing them can't
##     change the graph shape.
##   - [PruneNonEssential]: removes the given connections only if their removal
##     would not isolate any node (every node keeps at least one enabled
##     incoming and one enabled outgoing) AND every output remains reachable
##     from some input.
...
##     This is the ONLY prune mutator allowed to change graph shape.
...
## Base implementation: removes enabled connections only if safe (won't
## isolate any node). Disabled connections are removed unconditionally
## (they don't affect the graph).
func mutate(genome: Genome, conns: Array, ctx: MutationContext) -> Array[int]:
        ...
        for c_v: Variant in conns:
                ...
                if not genome.connections.has(c.innovation):
                        continue
                if c.enabled and not _is_safe_to_remove(genome, c):
                        continue
                genome.remove_connection(c.innovation)
                removed.append(c.innovation)
        return removed

## Check if removing [param conn] would leave its source node with no other
## enabled outgoing, or its target node with no other enabled incoming.
## Returns true if the removal is safe (neither endpoint gets isolated).
static func _is_safe_to_remove(genome: Genome, conn: ConnectionGene) -> bool:
        var from_has_other_out: bool = false
        var to_has_other_in: bool = false
        for c: ConnectionGene in genome.connections.values():
                if c.innovation == conn.innovation:
                        continue
                if not c.enabled:
                        continue
                if c.from_node == conn.from_node:
                        from_has_other_out = true
                if c.to_node == conn.to_node:
                        to_has_other_in = true
                if from_has_other_out and to_has_other_in:
                        return true
        return from_has_other_out and to_has_other_in
...
class PruneNonEssential:
        extends PruneMutator
        # Don't prune if it would isolate any node (node loses all enabled
        # incoming or all enabled outgoing) OR if it would disconnect an
        # output from all inputs.
        ...
        func mutate(genome: Genome, conns: Array, ctx: MutationContext) -> Array[int]:
                ...
                for c_v: Variant in conns:
                        ...
                        if not genome.connections.has(c.innovation):
                                continue
                        # Safety check: don't remove if it would isolate a node.
                        if c.enabled and not _is_safe_to_remove(genome, c):
                                continue
                        # Tentatively remove.
                        genome.remove_connection(c.innovation)
