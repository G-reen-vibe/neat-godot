...
## If a cycle is detected (should not happen when [member forbid_loops] is
## true), falls back to a timestep pass with enough steps to propagate the
## signal through a typical feedforward network.
func forward_topological(inputs: Dictionary, fallback_steps: int = 5) -> Dictionary:
		if not compute_topological_order():
				...
				# We fall back to a timestep pass to avoid crashing.
				return forward_timestep(inputs, fallback_steps)
		_activation_cache.clear()
		for n: NodeGene in nodes.values():
				if n.kind == NodeGene.Kind.INPUT:
						_activation_cache[n.id] = float(inputs.get(n.id, 0.0))
...
## [param steps] is used for timestep mode and as the fallback step count
## for topological mode if a cycle is detected.
func forward(inputs: Dictionary, mode: String = "topological", steps: int = 5) -> Dictionary:
		if mode == "timestep":
				return forward_timestep(inputs, steps)
		return forward_topological(inputs, steps)
