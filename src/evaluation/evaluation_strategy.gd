...
class Equal:
	...
	func evaluate(species_list: Array, total_population: int, _ctx: MutationContext) -> void:
		...
		var per := maxi(1, total_population / n)
		for sp: Species in species_list:
			sp.allocated_children = per
			sp.mutation_rate_multiplier = 1.0
		# Adjust for rounding on the first species.
		var total := per * n
		if total != total_population:
			(species_list[0] as Species).allocated_children += (total_population - total)
...
class ImprovementRate:
	...
	func evaluate(species_list: Array, total_population: int, _ctx: MutationContext) -> void:
		...
		if species_list[0].best_fitness_history.size() >= 2:
			kept = _cull_low_species(species_list, scores, min_score_for_survival)
		_allocate_proportional(kept, scores, total_population)
