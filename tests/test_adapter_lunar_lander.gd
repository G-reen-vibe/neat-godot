...
func _test() -> void:
        ...
        # Reward for OOB = -100. Per-step in-flight reward ≈ -0.2. Total should be
        # very negative (around -100 minus ~30 steps of -0.2 ≈ -106).
        _assert(fit < -50.0, "fitness should be very negative (OOB penalty -100 + in-flight costs), got %f" % fit)
        # Reset clears state.
        probe.reset(null, rng)
        await get_tree().physics_frame
