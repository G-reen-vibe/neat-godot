...
func _ready() -> void:
	...
	# Verify config screen has actual content (not blank).
...
# Also verify config screen has actual content (not blank).
func _test_config_content() -> void:
	print("  --- config content check ---")
	app._select_env(0)  # XOR
	assert(app._config_vbox != null, "Config vbox should exist")
	assert(app._config_vbox.get_child_count() > 0, "Config vbox should have children")
	var visible_count: int = 0
	for c in app._config_vbox.get_children():
		if c is Control and c.visible:
			visible_count += 1
	print("    XOR config: %d visible rows" % visible_count)
	assert(visible_count >= 10, "XOR config should have at least 10 visible rows, got %d" % visible_count)
