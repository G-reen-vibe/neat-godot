...
##
## Uses a custom _assert helper that halts on failure (Godot's built-in assert
## only logs an error but does not stop execution by default).
...
func _test_all_activation_functions() -> void:
        for f in ActivationFunctions.all_ids():
                ...
                _assert(not is_nan(v), "Activation %d returned NaN" % f)
                var v2: float = ActivationFunctions.activate(f, -0.5)
                _assert(not is_nan(v2), "Activation %d returned NaN for negative input" % f)
                # Name round-trip.
                var n := ActivationFunctions.name_of(f)
                _assert(ActivationFunctions.from_name(n) == f, "Activation name round-trip failed for %d" % f)
...
func _test_all_randomization_methods() -> void:
        ...
        for method in ["gaussian", "triangular", "roulette", "inverse_roulette", "uniform"]:
                ...
                _assert(picked != null, "Method %s returned null" % method)
                _assert(items.has(picked), "Method %s returned invalid item" % method)
...
func _test_all_weight_selectors() -> void:
        ...
        _assert(not out1.is_empty(), "Standard weight selector returned empty")
        ...
        _assert(not out2.is_empty(), "Capped weight selector returned empty")
...
func _test_all_weight_mutators() -> void:
        ...
        wm1.mutate(c, ctx)
        _assert(absf(c.weight - w_before) < 1.0, "Standard weight mutator changed weight too much")
        # Normal.
        ...
        _assert(not is_nan(c2.weight), "Normal weight mutator produced NaN")
...
func _test_all_prune_selectors() -> void:
        ...
        for ps_class in [PruneSelector.Standard, PruneSelector.LeastWeight]:
                ...
                _assert(not out.is_empty(), "Prune selector returned empty")
...
func _test_all_prune_mutators() -> void:
        ...
        _assert(out1.size() > 0, "PruneDisabled should remove disabled connections")
        # PruneNonEssential.
        var g2 := _build_starter_genome()
...
func _test_all_neuron_crossovers() -> void:
        ...
        for nc_class in [NeuronCrossover.Standard, NeuronCrossover.Average, NeuronCrossover.BiasedAverage]:
                ...
                _assert(child.connection_count() == 3, "Neuron crossover should preserve connection count")
        # StandardAll.
        var nc4 := NeuronCrossover.StandardAll.new()
        var oc4 := OverallCrossover.Fitter.new(nc4)
        var child4 := oc4.crossover(a, b, ctx)
        _assert(child4.connection_count() == 3, "StandardAll should preserve connection count")
...
func _test_all_overall_crossovers() -> void:
        ...
        for oc_class in [OverallCrossover.Fitter, OverallCrossover.Bigger, OverallCrossover.Combine, OverallCrossover.Excluded]:
                ...
                var child := oc.crossover(a, b, ctx)
                _assert(child.node_count() >= 4, "Overall crossover should preserve at least the shared nodes")
                _assert(not child.has_loop(), "Overall crossover should not create loops")
...
func _test_all_similarity_tests() -> void:
        ...
        for sim_class in [SimilarityTest.Standard, SimilarityTest.Percentage]:
                ...
                _assert(d >= 0.0, "Similarity distance should be non-negative")
                _assert(not is_nan(d), "Similarity distance should not be NaN")
...
func _test_all_speciation_strategies() -> void:
        ...
        for sp_class in [SpeciationStrategy.Single, SpeciationStrategy.Standard]:
                ...
                _assert(not species.is_empty(), "Speciation should produce at least 1 species")
        # KMedian.
        var km := SpeciationStrategy.KMedian.new(3, 3)
        var species_km: Array = km.speciate(genomes.duplicate(true), [], sim, ctx)
        _assert(species_km.size() <= 3, "KMedian with k=3 should produce <= 3 species")
        # Purge: first-gen should produce target_species_count species (default 10),
        # clamped to the number of genomes (20 here). So expect 10 species.
        ...
        var purge := SpeciationStrategy.Purge.new(pol)  # default target = 10
        var species_purge: Array = purge.speciate(genomes.duplicate(true), [], sim, ctx)
        var expected_purge: int = mini(10, genomes.size())
        _assert(species_purge.size() == expected_purge, "Purge first-gen should produce %d species, got %d" % [expected_purge, species_purge.size()])
...
func _test_all_evaluation_strategies() -> void:
        ...
        for ev_class in [EvaluationStrategy.Equal, EvaluationStrategy.ImprovementRate]:
                ...
                _assert(total == 30, "Total allocated should be 30, got %d" % total)
...
func _test_all_generation_strategies() -> void:
        ...
        _assert(children1.size() == 20, "Asexual should produce 20 children, got %d" % children1.size())
        # Crossover.
        var nc := NeuronCrossover.Standard.new()
        var oc := OverallCrossover.Fitter.new(nc)
        var gs2 := GenerationStrategy.Crossover.new(pol, oc)
        var children2: Array = gs2.produce(species_list.duplicate(true), ctx)
        _assert(children2.size() == 20, "Crossover should produce 20 children, got %d" % children2.size())
        # Mixed.
        var gs3 := GenerationStrategy.Mixed.new(pol, oc, 0.5)
        var children3: Array = gs3.produce(species_list.duplicate(true), ctx)
        _assert(children3.size() == 20, "Mixed should produce 20 children, got %d" % children3.size())
...
func _test_all_mutation_policies() -> void:
        ...
        _assert(not g.has_loop(), "General policy should not create loops")
        # Phased.
        var g2 := _build_starter_genome()
...
func _test_forward_modes_match() -> void:
        ...
        for k in out_topo.keys():
                ...
                _assert(diff < 1e-4, "Topo and timestep should match for feedforward, diff=%f" % diff)
...
func _test_loop_prevention() -> void:
        ...
        for _i in range(20):
                pol.apply(g, ctx)
        _assert(not g.has_loop(), "After 20 mutations with forbid_loops, no loop should exist")
...
func _test_xor_with_crossover() -> void:
        ...
        for _i in range(5):
                ...
                pop.evolve()
        _assert(pop.size() == cfg.population_size, "XOR+crossover: pop size stable")
...
func _test_xor_with_mixed_generation() -> void:
        ...
        for _i in range(5):
                ...
                for i in range(pop.genomes.size()):
                        pop.genomes[i].fitness = fitnesses[i]
                pop.evolve()
        _assert(pop.size() == cfg.population_size, "XOR+mixed: pop size stable")
...
func _test_xor_with_phased_pruning() -> void:
        ...
        for _i in range(5):
                ...
                for i in range(pop.genomes.size()):
                        pop.genomes[i].fitness = fitnesses[i]
                pop.evolve()
        _assert(pop.size() == cfg.population_size, "XOR+phased: pop size stable")
...
func _test_xor_with_improvement_rate() -> void:
        ...
        for _i in range(5):
                ...
                for i in range(pop.genomes.size()):
                        pop.genomes[i].fitness = fitnesses[i]
                pop.evolve()
        _assert(pop.size() == cfg.population_size, "XOR+improvement: pop size stable")
...
func _test_xor_with_novelty() -> void:
        ...
        for _i in range(5):
                ...
                for i in range(pop.genomes.size()):
                        pop.genomes[i].fitness = fitnesses[i]
                pop.evolve()
        _assert(pop.size() == cfg.population_size, "XOR+novelty: pop size stable")
...
func _test_parallel_vs_serial() -> void:
        ...
        for i in range(fit_serial.size()):
                _assert(absf(fit_serial[i] - fit_parallel[i]) < 1e-3, "Parallel vs serial mismatch at %d" % i)
