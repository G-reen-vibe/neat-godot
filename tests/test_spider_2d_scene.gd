extends Node
## Phase 3 test: Spider 2D with real multi-body physics.
## Run with: godot --headless --path . res://tests/test_spider_2d_scene.tscn

const MAX_GENERATIONS: int = 8
...
func _ready() -> void:
	print("=== test_spider_2d_scene: real-physics Spider 2D via SceneEvaluator ===")
	await _test_spider_2d()
	if _failed:
		printerr("\n=== test_spider_2d_scene: FAILED ===")
		get_tree().quit(1)
	else:
		print("\n=== test_spider_2d_scene: PASSED ===")
		get_tree().quit(0)
...
func _test_spider_2d() -> void:
	...
	pop.initialize()
	var env_scene: PackedScene = load("res://environments/spider_2d/spider_walker_2d_environment.tscn")
