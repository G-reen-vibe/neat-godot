...
func _ready() -> void:
        ...
        # Recalculate grid columns when the scroll container is resized
        # (covers initial layout + window resize).
        _viz_scroll.resized.connect(_recalc_columns)
...
func _build_grid() -> void:
        ...
        for i in range(n):
                ...
                # reparent() is atomic (no brief detach from tree), safer than
        ...
        # Defer column calculation to the next frame, when the layout has
        # been computed and _viz_scroll has its final width.
        call_deferred("_recalc_columns")

## Recalculate the number of grid columns based on the scroll container's
## width. Called on initial layout (deferred) and on resize.
func _recalc_columns() -> void:
        if not is_instance_valid(_viz_scroll):
                return
        var available_width: float = _viz_scroll.size.x
        if available_width < CELL_SIZE.x:
                return  # layout not ready yet; will be called again on resized
