...
func _is_solved() -> bool:
                ...
                # Note: the Evaluator returns AVERAGE fitness over episodes, so the max
                # possible fitness is a single episode's max (not episodes * max_steps).
                match _env_idx:
...
func _make_config(env_idx: int) -> NeatConfig:
                ...
                # --- Generation: Mixed (crossover + asexual) with elitism (standard NEAT) ---
                ...
                # Note: crossover_rate is set in the Generation section above (0.75).
