...
func _ready() -> void:
	...
	_best_btn.toggled.connect(func(p): _show_best = p; _refresh())
...
func _prev_species_click() -> void:
	...
	_current_genome_idx = 0
	_show_best = false
	_best_btn.button_pressed = false
	_refresh()
...
func _refresh() -> void:
	...
	if _show_best and population.best_genome != null:
		_species_label.text = "Best Genome"
		_genome_label.text = "(snapshot)"
		_graph_view.genome = population.best_genome
		var g: Genome = population.best_genome
		_stats_label.text = "Best Genome: nodes=%d conns=%d fit=%.3f" % [
	...
	_stats_label.text = "nodes=%d conns=%d fit=%.3f\nspecies: members=%d best=%.3f avg=%.3f stale=%d" % [
		g.node_count(), g.connection_count(), g.fitness,
