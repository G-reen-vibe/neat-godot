...
##   - Follow toggle: F key, or set_auto_follow(bool)
extends Control
class_name EnvViewport

const CAMERA_PAN_SPEED: float = 24.0
...
func is_auto_follow() -> bool:
	return _auto_follow

func set_auto_follow(p: bool) -> void:
	_auto_follow = p
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed:
		match event.keycode:
			KEY_LEFT:
				_camera_offset.x += CAMERA_PAN_SPEED
				_auto_follow = false
			KEY_RIGHT:
				_camera_offset.x -= CAMERA_PAN_SPEED
				_auto_follow = false
			KEY_UP:
				_camera_offset.y += CAMERA_PAN_SPEED
				_auto_follow = false
			KEY_DOWN:
				_camera_offset.y -= CAMERA_PAN_SPEED
				_auto_follow = false
...
func _draw() -> void:
	...
	# Camera help.
	draw_string(ThemeDB.fallback_font, Vector2(8, size_vec.y - 6), "arrows=pan  +/-/0=zoom  F=follow", HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color(0.4, 0.4, 0.4))
