...
func _draw_cartpole(state: Dictionary, center: Vector2, zoom: float, size_vec: Vector2) -> void:
	...
	var pole_pos: Vector2 = state.get("pole_pos", Vector2.ZERO)
	...
	# Track line (at y = track_y in world coords).
	...
	var cart_w: float = 40.0 * zoom
	var cart_h: float = 16.0 * zoom
	...
	# Pole: from pivot (top of cart) upward at angle theta.
	# In RL env, pole hangs from cart via PinJoint. Pole position is the pole's
	# center; the pivot is at pole_pos.y - pole_half_len (top of pole).
	# Pole visual: a line from pivot to bottom of pole, rotated by theta.
	var pivot_screen := _world_to_screen_px(Vector2(cart_pos.x, cart_pos.y - 30.0), center, zoom)
	# Pole extends downward from pivot by pole_half_len * 2.
	var pole_end := pivot_screen + Vector2(sin(theta), cos(theta)) * (pole_half_len * 2.0 * zoom)
